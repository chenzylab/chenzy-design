...
- **间距语义化**：`spacing` 默认提供三档语义值，映射到全局间距 Token，保证全站间距节奏一致（对齐 Semi）：
  - `tight` → `--cd-space-tight`（紧凑，密集信息区，8px，组件默认值）
  - `medium` → `--cd-space-medium`（中等，16px）
  - `loose` → `--cd-space-loose`（宽松，分区/卡片操作区，24px）
...
- **零包裹副作用**：Space 默认 `inline-flex`（横向）以便随内容收缩；提供 `block` 占满父宽时切 `flex`。子元素不被强制包裹额外 DOM。
...
| `@chenzy-design/svelte` | 全部 | `Space.svelte` 负责：解析 `spacing` 为 CSS 变量、拼装 flex 样式、透传根节点属性（`class`/`style`/`...$$restProps`）。 |
...
| `block` | `boolean` | `false` | 是否占满父容器宽度（`display: flex` 而非 `inline-flex`）。（Semi 无，超集能力） |
| `tag` | `keyof HTMLElementTagNameMap` | `'div'` | 根元素标签，便于语义化（如 `ul`/`nav`）。（Semi 无，超集能力） |
...
| --- | --- | --- |
| `--cd-space-tight` | `var(--cd-spacing-tight)` = 8px（对齐 Semi） | `tight` 档位间距（默认档） |
| `--cd-space-medium` | `var(--cd-spacing-base)` = 16px（对齐 Semi） | `medium` 档位间距 |
| `--cd-space-loose` | `var(--cd-spacing-loose)` = 24px（对齐 Semi） | `loose` 档位间距 |
...
- **单元测试（Vitest）**：
  ...
  - `vertical` 切换正确设置 `flex-direction`（false→row / true→column）。
  - `align` 映射 `align-items`（start→flex-start 等）。
  - `wrap` 控制 `flex-wrap`（`vertical` 时强制不换行）。
  - `block` 切换 `inline-flex`/`flex`。
  - `tag` 渲染指定根标签。
  - 不产生额外包裹 DOM（纯 `gap` 快路径）。
...
- [ ] 类名遵循 `cd-space`、`cd-space--vertical` / `cd-space--horizontal` / `cd-space--wrap` / `cd-space--block` 等 BEM-like 约定。
- [ ] `vertical` / `spacing` / `align` / `wrap` / `block` / `tag` 行为符合第 4 节定义。
