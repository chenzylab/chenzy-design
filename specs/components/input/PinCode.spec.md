...
- **焦点管理**：各格均可 Tab 到达（与 Semi 一致，不做单一 roving 停靠点——OTP 场景用户期望能直接点任意格修正）。autoFocus 默认开启（对齐 Semi，见 §4）；如页面有多个 PinCode 或需避免抢焦点，显式传 `autoFocus={false}`。
...
- `props`/`events` schema（类型、默认值、枚举、受控标记）。
