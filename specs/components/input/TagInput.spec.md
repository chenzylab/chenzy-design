...
| prefix | `string` | — | 前缀图标/文本（slot 优先）。 |
| suffix | `string` | — | 后缀图标/文本（slot 优先）。 |
| renderTagText | `(tag: string) => string` | — | 自定义标签显示文本（不影响 value）。 |
...
| tag | `{ tag, index, remove, disabled }` | 自定义单个标签渲染（覆盖默认 Tag）。 |
| removeIcon | `{ index }` | 自定义标签删除图标。 |
| clearIcon | — | 自定义清空按钮图标。 |
...
| 维度 | 预算 / 目标 | 说明 |
|---|---|---|
| gzip 体积（svelte） | ≤ 3.5 KB | 不含共享 Tag/Input 基元与 core。 |
