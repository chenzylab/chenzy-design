...
- **触发区（trigger）**：按钮型或拖拽区型。拖拽区为虚线边框矩形，默认 `--cd-color-border`，hover/drag-over 时边框与背景切换为 `--cd-color-primary` 语义的浅色派生（`--cd-upload-drag-active-bg`）。
- **状态语义映射**：
  - uploading → 中性信息色 + 进度条（`--cd-color-primary`）。
  - success → 成功色（Alias `--cd-color-success` 派生的 `--cd-upload-status-success`）。
  - error / validateFail → 危险色 `--cd-color-danger`，文件项整体描边变红，附错误文案。
  - waiting → 次级文本 `--cd-color-text-2`。
...
- 文件项状态机：`waiting → validating →(fail)validateFail | (pass)uploading → success | error`，error/validateFail 可 `retry()` 回到 `waiting`。
...
| `limit` | `number` | — | 列表最大文件数；超出阻止新增并 `on:exceed` |
...
| `data` | `Record<string,any> \| (file)=>...` | — | 附加表单字段 |
| `name` | `string` | `'file'` | 表单文件字段名 |
...
| `renderFileItem` | `Snippet<[{ fileItem, remove, retry, preview }]>` | — | 完全自定义单个文件项渲染，替换默认列表项（list/text listType）。对标 Semi renderFileItem |
...
| `renderPicPreviewIcon` | `Snippet<[{ fileItem, remove, retry, preview }]>` | — | 自定义照片墙 hover 预览图标（picture-card），常配合 `onPreviewClick`。对标 Semi renderPicPreviewIcon |
| `renderPicClose` | `Snippet<[{ className, remove }]>` | — | 自定义照片墙关闭（移除）按钮（picture-card）。对标 Semi renderPicClose |
| `renderFileOperation` | `Snippet<[{ fileItem, remove, retry, preview }]>` | — | 自定义文件列表项操作区（text/list，替换默认重试/移除）。对标 Semi renderFileOperation |
...
| `itemStyle` | `string \| Record<string, string>` | — | 每个文件列表项/卡片的自定义 style（合并进容器）。对标 Semi itemStyle |
...
- [ ] Perf Budget 达标：svelte ≤6KB / core ≤4KB gzip；大列表虚拟化；progress rAF 节流；objectURL 释放；预览 destroyOnClose。
