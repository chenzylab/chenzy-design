...
| `showTooltip` | `boolean \| { type?: 'tooltip'\|'popover'; opts?: Record<string, unknown>; renderTooltip?: Snippet<[{ content }]> }` | `true` | 文件名提示：boolean=原生 `title` 属性；对象=用 Tooltip/Popover 包裹文件名，`renderTooltip` 自定义提示内容。对标 Semi showTooltip |
