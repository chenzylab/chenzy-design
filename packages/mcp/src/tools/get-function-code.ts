...
- ${SVELTE_PACKAGE}/dist/table/Table.svelte`,
  inputSchema: {
    type: 'object',
    properties: {
      filePath: { type: 'string', description: `文件完整路径，如 ${CORE_PACKAGE}/src/table.ts` },
...
export async function handleGetFunctionCode(args: Record<string, unknown>): Promise<CallToolResult> {
  ...
  if (!functionName) {
    return { content: [{ type: 'text', text: '错误：请提供函数名称 (functionName)' }], isError: true };
  }
  const parsed = parseFilePath(filePath);
  if (!parsed) {
    return {
      content: [
        { type: 'text', text: `错误：无效的文件路径格式。路径应为 ${SVELTE_PACKAGE}/xxx 或 ${CORE_PACKAGE}/xxx 格式。\n\n提供的路径: ${filePath}` },
      ],
      isError: true,
    };
  }
  ...
  try {
    ...
    return {
      content: [
        {
          type: 'text',
          text: [`文件: ${filePath}`, `函数: ${functionName}`, `版本: ${version}`, '', '='.repeat(60), '', functionCode].join('\n'),
        },
      ],
    };
  } catch (error) {
