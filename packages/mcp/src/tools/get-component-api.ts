  ...
  inputSchema: {
    ...
    properties: {
      componentName: {
        type: 'string',
        description:
          '组件名，如 Button、Table、date-picker（大小写/连字符不敏感）',
      },
      section: {
        type: 'string',
        enum: [...SECTIONS],
...
export function renderApi(meta: ComponentMeta, section: Section): string {
  ...
  if (want('props') && meta.props?.length) {
    parts.push(
      `## Props`,
      fieldTable(meta.props, ['属性', '说明', '类型', '默认值']),
    );
  }
  if (want('events') && meta.events?.length) {
    parts.push(
      `## Events`,
      fieldTable(meta.events, ['事件', '说明', '类型', '默认值']),
    );
  }
  if (want('slots') && meta.slots?.length) {
    parts.push(
      `## Slots (snippets)`,
      fieldTable(meta.slots, ['插槽', '说明', '类型', '默认值']),
    );
  }
  ...
  if (section === 'all' && meta.subComponents?.length) {
    for (const sub of meta.subComponents) {
      ...
      if (sub.props?.length)
        parts.push(fieldTable(sub.props, ['属性', '说明', '类型', '默认值']));
    }
  }
  ...
  if (!SECTIONS.includes(section)) {
    return {
      content: [
        {
          type: 'text',
          text: `错误: section 必须是 ${SECTIONS.join('/')} 之一`,
        },
      ],
      isError: true,
    };
  ...
  try {
    ...
    if (resolved.viaSubComponent) {
      const sub = resolved.meta.subComponents?.find(
        (s): s is Exclude<typeof s, string> =>
          typeof s !== 'string' && s.name === resolved.viaSubComponent,
      );
      if (sub) {
    ...
    return {
      content: [
        {
          type: 'text',
          text: `# ${resolved.metaName}（版本 ${manifest.version}）\n\n${resolved.meta.description ?? ''}\n\n${body}`,
        },
      ],
    };
  } catch (error) {
    return {
      content: [
        {
          type: 'text',
          text: `获取组件 API 失败: ${error instanceof Error ? error.message : String(error)}`,
        },
      ],
      isError: true,
    };
