    ...
    switch (node.type) {
      ...
      case 'MethodDefinition': {
        if (node.key && node.value && typeof node.value === 'object') {
          ...
          funcName =
            keyNode.name || (keyNode as unknown as { value: string }).value;
          const valueNode = node.value as ASTNode;
          if (
            valueNode.body &&
            typeof valueNode.body === 'object' &&
            !Array.isArray(valueNode.body)
          ) {
 ...
 * 处理变量声明中的函数赋值，提取函数名
 */
function extractVariableDeclarationFunctions(
  ast: ASTNode,
  code: string,
      ...
      for (const decl of node.declarations as ASTNode[]) {
        if (decl.type === 'VariableDeclarator' && decl.id && decl.init) {
          ...
          if (
            ...
            initNode.body &&
            typeof initNode.body === 'object' &&
            !Array.isArray(initNode.body) &&
 ...
 * 解析代码并获取所有函数
 */
export function findAllFunctions(
  code: string,
  filename: string = 'code.tsx',
): FunctionInfo[] {
  try {
 ...
 * @returns 替换后的代码
 */
export function removeFunctionBodies(
  code: string,
  filename: string = 'code.tsx',
): string {
  ...
  // 过滤掉嵌套的函数，只处理非嵌套的函数体
  // 从最深层开始替换，避免位置偏移问题
  const sortedByBodyStart = [...functions].sort(
