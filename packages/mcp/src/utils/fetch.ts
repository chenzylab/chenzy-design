...
export async function fetchFileContentFromSource(
  ...
  if (
    ...
    content.includes('npmmirror 镜像站')
  ) {
...
export async function fetchFileContent(
  ...
  const unpkgPromise = fetchFileContentFromSource(
    UNPKG_BASE_URL,
    packageName,
    resolvedVersion,
    filePath,
  ...
  const npmmirrorPromise = fetchFileContentFromSource(
    NPMMIRROR_BASE_URL,
    packageName,
    resolvedVersion,
    filePath,
  ...
  const raceResult = await Promise.any([unpkgPromise, npmmirrorPromise]).catch(
    () => null,
  );
...
async function fetchNpmMirrorDirectoryRecursive(
  ...
  if (!response.ok)
    throw new Error(
      `获取目录列表失败: ${response.status} ${response.statusText}`,
    );
  ...
  if (Array.isArray(data.files)) {
    ...
    for (const item of data.files) {
      ...
      if (
        ...
        const subPath = normalized.path.startsWith('/')
          ? normalized.path.slice(1)
          : normalized.path;
        promises.push(
          fetchNpmMirrorDirectoryRecursive(
            baseUrl,
            packageName,
            version,
            subPath,
            ...
            .then((subFiles) =>
              subFiles.filter((f) => f.path !== normalized.path),
            )
            .catch(() => []),
        );
      } else if (item.files?.length) {
        const flattened = flattenNested(item).filter(
    ...
    if (promises.length) {
      for (const subFiles of await Promise.all(promises))
...
export async function fetchDirectoryListFromSource(
  ...
  if (isNpmMirror)
    return fetchNpmMirrorDirectoryRecursive(
      baseUrl,
      packageName,
      version,
      path,
...
export async function fetchDirectoryList(
  ...
  if (results[0].status === 'fulfilled')
    ok.push({ source: 'unpkg', files: results[0].value });
  else
    errors.push(
      String(
  ...
  if (results[1].status === 'fulfilled')
    ok.push({ source: 'npmmirror', files: results[1].value });
  else
    errors.push(
      String(
