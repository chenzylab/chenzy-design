...
借助 `svelte-dnd-action` 的 `dndzone`，通过 `bodyAttach` 挂到表体（tbody）即可实现整表行拖拽排序，支持鼠标与键盘（无障碍）两种操作。

<Notice type="primary" title="与 Semi 的差异">

Semi 借助 React 的 `dnd-kit` 搭配 `components.body.row` 注入自定义 `SortableRow` 组件实现拖拽排序。本库为 Svelte，`components` 仅支持覆盖组成元素的**标签名字符串**（不接受自定义组件），故改为提供 `bodyAttach` prop——把 `svelte-dnd-action` 的 `dndzone` 以 [attachment](https://svelte.dev/docs/svelte/@attach) 形式挂到 tbody，在 `consider`/`finalize` 事件里更新 `dataSource` 顺序（技术栈差异：`dnd-kit` → `svelte-dnd-action`，可观察结果一致）。
...
| bodyAttach | 挂到表体（tbody）的 [attachment](https://svelte.dev/docs/svelte/@attach)，用于接入拖拽库（如 `svelte-dnd-action` 的 `dndzone`）实现行拖拽排序 | Attachment | - |
