...
通过 `onRow` 返回 `draggable` 及 HTML5 拖拽事件（`onDragStart`/`onDrop` 等）可实现拖拽排序。

<Notice type="primary" title="与 Semi 的差异">

Semi 借助 React 的 `dnd-kit` 搭配 `components.body.row` 注入自定义 `SortableRow` 组件实现拖拽排序。本库为 Svelte，`components` 仅支持覆盖组成元素的**标签名字符串**（不接受自定义组件），故拖拽排序改由 `onRow` 返回原生拖拽属性（`draggable` + HTML5 拖拽事件）实现——由 Table 独立渲染表格行、拖拽事件仅更新 `dataSource` 顺序，避免拖拽库与组件争夺 DOM 控制权（技术栈差异，可观察结果一致）。
