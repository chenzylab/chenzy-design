...
- **简短单一信息点**：一个 Tooltip 只承载一个信息点，避免整段说明（超过 2 行考虑改用 Popover）。
- **不重复可见文字**：不复述触发元素已有的可见文案，避免冗余。
- **不放可点击内容**：链接、按钮等需点击的内容在 Tooltip 内不可达，禁止放置。
- **解释禁用原因**：用于说明危险操作为何被禁用时，给出原因与解锁条件，措辞明确不模糊。

| ✅ 推荐用法 | ❌ 不推荐用法 |
| --- | --- |
| 需先取消关联项目后才能删除 | 该操作可能无法使用 |
| 复制链接 | 点击此按钮可以复制链接到剪贴板 |
| 短提示无句末标点 | 短提示。 |
