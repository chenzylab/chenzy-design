...
- **动作明确**："上一页/下一页"用动作短语，不用"前进/后退"等歧义词。
- **数字统一格式化**：经 Intl 格式化（如 `1,234`），避免裸大数。
- **simple 模式保持语序**：`第 X / Y 页` 维持"当前/总数"语序，禁止省略总数。
- **越界提示中性**：用"页码超出范围"，不指责用户。

| ✅ 推荐用法 | ❌ 不推荐用法 |
| --- | --- |
| 上一页 / 下一页 | 后退 / 前进 |
| 第 5 / 20 页 | 第 5 页 |
| 第 41-50 条 / 共 1,234 条 | 第 41-50 条 / 共 1234 条 |
| 页码超出范围 | 你输错了页码 |

- Pagination 不含破坏性操作；切换 pageSize 以提示而非警告口吻通告"列表已重新计算"。
