...
- **占位文案动词在前**、说明操作方式，如「请输入后按回车」，避免「标签输入框」这类描述控件本身的文案。
- **计数与上限**用「已添加 N 个」「最多可添加 N 个」，统一「个」量词，不混用「项/条」。
- **校验提示简短可执行**，如「{tag} 已存在」而非长句解释。
- 播报文案与可见文案语义一致。

| ✅ 推荐用法 | ❌ 不推荐用法 |
| --- | --- |
| 请输入后按回车 | 标签输入框 |
| 已添加 3 个 / 最多可添加 5 个 | 已添加 3 项 / 最多 5 条 |
| react 已存在 | 您输入的标签与已有标签重复，请重新输入 |

- 清空全部默认不二次确认（轻量删除可逆性由宿主撤销机制承担）；标签数达阈值时确认文案为「清空全部标签？此操作不可撤销。」。
