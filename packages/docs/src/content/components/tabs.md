...
- **标题简短**：用名词或名词短语，建议 1–4 字 / 1–2 词，避免句子；同组词性一致。
- **辅助文案简洁**：新增/更多等用动词或简洁名词。
- **不放长说明**：标签内不放长说明文字，需要描述时放入面板内容区。

| ✅ 推荐用法 | ❌ 不推荐用法 |
| --- | --- |
| 概览 / 详情 / 设置 | 这是概览页（含说明文字） |
| 新增 | 点击新增一个标签页 |

- 关闭标签为潜在数据丢失操作，含未保存内容时应在 `on:close` 拦截弹确认；关闭叉 `aria-label` 明确为"关闭 {tab}"。
