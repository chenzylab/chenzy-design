...
树形控件（Tree）以层级缩进的方式展示具有父子关系的结构化数据，支持展开/收起节点、单选/多选、勾选（带半选/全选级联）、搜索过滤、拖拽排序等能力。典型使用场景：文件目录树、组织架构、权限菜单配置、分类导航。
...
- **系统文案简洁中性**：空态/加载等系统文案句末不加句号，搜索无结果复用空态文案。
- **计数统一**：用"已选 N 项"，避免"个/条"混用，统一 ICU plural。
- **操作动词成对**：用"展开/收起"成对、"全选"而非"勾选全部"。

| ✅ 推荐用法 | ❌ 不推荐用法 |
| --- | --- |
| 暂无数据 | 暂无数据。 |
| 暂无匹配结果 | 没有找到任何东西！ |
| 已选 3 项 | 已选 3 个 / 已选 3 条 |
| 全选 | 勾选全部 |

- 业务若在 `suffix` 放删除按钮，须二次确认，建议文案"删除「{label}」及其全部子节点？此操作不可撤销。"。
