...
- **标签简洁**，用名词或动名词短语，不加句末标点；避免否定式，用「启用通知」而非「不禁用通知」。
- **协议类场景**标签陈述用户行为，如「我已阅读并同意服务条款」，链接内联。
- **辅助文案用 extra** 补充说明，颜色弱化；校验错误提示放组件外（如 Form.Item）。
- 控制破坏性后果时文案需明示后果，不依赖颜色。
...
- 「不再提示」类降低安全感的项默认不勾选，由用户主动选择；此类项不放在大命中区诱导误触。
