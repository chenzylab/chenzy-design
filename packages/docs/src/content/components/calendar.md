...
- **省略号仅用于进行态**：如 "Loading events…"。
- **时间范围用 en-dash**：用 `–` 连接，左右留空格。
- **空态友好中性**："No events" 不加感叹号。

| ✅ 推荐用法 | ❌ 不推荐用法 |
| --- | --- |
| Today | Now |
| Month / Week / Day | 月视图切换 |
| No events | 没有任何事件！！ |
| `+3 more` | 还有另外 3 个更多的事件 |

- 业务接入删除时，确认文案须由 Popconfirm/Modal 实现，如 "Delete event "{title}"? This can't be undone."。
