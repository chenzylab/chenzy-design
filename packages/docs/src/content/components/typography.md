...
- **动作文案用动词原形**：复制、编辑、展开、收起，简短。
- **提示态确认性**：用"已复制"而非"复制成功了！"。
- **展开/收起成对对称**：默认"展开"/"收起"，避免风格不一致。
- **省略号统一**：截断统一用单字符 `…`（U+2026），不用三个点。
...
- `type="danger"` 仅表达视觉语义，不承载删除动作；危险确认由上层 Modal/Popconfirm 承担。
