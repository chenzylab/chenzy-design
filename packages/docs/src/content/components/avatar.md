...
<script>
  ...
  import Rtl from '../../demos/avatar/15-rtl.svelte';
  import rtlSrc from '../../demos/avatar/15-rtl.svelte?raw';
</script>
...
通过 `ConfigProvider` 的 `direction="rtl"` 建立方向作用域：RTL 下头像组层叠方向、hover 遮罩定位随之镜像。

<DemoBox code={rtlSrc}><Rtl /></DemoBox>
