...
- **标题简短平行**：简短动宾或名词短语，建议 ≤ 6 字，如「填写信息」「确认订单」，平行结构、时态一致。
- **描述一句话**：补充该步要做什么，句末不加句号（除非多句）。
- **状态文案给出动作**：error/warning 步说明问题与下一步动作，而非仅"错误"。
- **避免编号混用**：序号由组件提供，标题不再混入编号。

| ✅ 推荐用法 | ❌ 不推荐用法 |
| --- | --- |
| 填写信息 | 第一步：请在这里填写您的相关信息 |
| 身份证号格式有误，请检查后重试 | 错误 |
| 确认订单 | 1. 确认订单 |

- 某步代表不可逆动作（如「提交并扣款」）时，title/description 须明确后果，实际确认交由表单按钮或 Modal。
