...
核心能力（严格对齐 Semi）：`data` 由用户按输入 `query` 准备（组件不做本地过滤，`onSearch` 存在即远程模式）、受控/非受控、候选项自定义渲染与关键词高亮、loading/empty 态、键盘交互、尺寸与校验态。
...
- 输入框使用 `role="combobox"`，配合 `aria-expanded`、`aria-controls`、`aria-autocomplete="list"`、`aria-activedescendant`。
- 候选面板使用 `role="listbox"`，每项使用 `role="option"` 并配合 `aria-selected`、`aria-disabled`。
- 键盘交互：`↓/↑` 移动高亮项（焦点常驻输入框）；`Enter` 选中当前高亮项；`Esc` 关闭面板保留文本。
