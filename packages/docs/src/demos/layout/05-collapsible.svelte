<script lang="ts">
  import { Layout, Nav } from '@chenzy-design/svelte';
...
<div style="border: 1px solid var(--cd-color-border); border-radius: 8px; overflow: hidden; height: 320px;">
  <Layout hasSider style="height: 100%;">
    <Layout.Sider
      ...
      style="background: var(--cd-color-bg-1); border-inline-end: 1px solid var(--cd-color-border); transition: width .2s;"
    >
      <Nav
        mode="vertical"
        isCollapsed={collapsed}
        onCollapseChange={(v) => (collapsed = v)}
    ...
    <Layout.Content padding style="background: var(--cd-color-bg-0); color: var(--cd-color-text-2);">
      点击侧栏底部按钮折叠 / 展开导航。Nav 内置 logo 头部、子导航展开、收起按钮联动。
    </Layout.Content>
  </Layout>
</div>
