<script lang="ts">
  ...
  const rowSelection = {
    ...
    onSelect: (record: FileRow, selected: boolean) => console.log(`select row: ${selected}`, record),
    onSelectAll: (selected: boolean, rows: FileRow[]) => console.log(`select all rows: ${selected}`, rows),
    onChange: (keys: (string | number)[], rows: FileRow[]) => console.log(`selectedRowKeys: ${keys}`, rows),
  };
</script>
