...
export const demos: DemoEntry[] = [
  ...
  entry('03-range.svelte', '范围选择', 'DatePicker 配合 disabledDate 与 maxRange 限制可选区间。'),
  ...
  entry('06-datetime-range.svelte', '带时间范围选择', 'DatePicker type=dateTimeRange 双面板 + 起止时/分/秒列 + 确认提交，支持 showSecond、disabledTime。'),
  entry('07-month-range.svelte', '月份范围选择', 'DatePicker type=monthRange 双月份面板（头部年份 + 左右切年）选起止月，value 为 [起始月, 结束月]。'),
  ...
  entry('13-range-presets.svelte', '范围快捷区间', 'DatePicker presets 提供最近 7 天等快捷区间，presetPosition 控制位置。'),
  entry('14-week-select.svelte', '周选择', 'DatePicker startDateOffset/endDateOffset 单击任意日期即选中整周。'),
];
