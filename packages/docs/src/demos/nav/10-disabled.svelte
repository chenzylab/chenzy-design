<script lang="ts">
  ...
  // 禁用项：覆盖三种粒度 —— 一级项禁用、子导航(Sub)禁用、子项禁用。
  // 对齐 Semi DisabledNav story。
  const items = [
    ...
    { itemKey: 'union', text: '公会中心', icon: iconStar },
    {
      ...
      disabled: true,
      items: [
        { itemKey: 'job_manage', text: '任务管理', disabled: true },
    ...
    },
    {
      ...
      icon: iconStar,
      items: [
        { itemKey: 'like', text: '我的喜欢', disabled: true },
...
<div style="width: 240px; height: 360px; border: 1px solid var(--cd-color-border); border-radius: 8px; overflow: hidden;">
  <Nav
    mode="vertical"
