...
export const demos: DemoEntry[] = [
  ...
  entry('05-source-a11y.svelte', '自定义图标', '通过 <Icon> 容器把外部 svg 纳入统一尺寸/颜色/旋转/a11y 体系；提供 type 后 role=img + aria-label；fill 覆盖填充色。'),
  // —— 本库补充（Semi 无）：<Icon> 容器尺寸/语义色 + UnoCSS 纯 class 路径 ——
  entry('01-basic.svelte', '容器尺寸（<Icon>）', '<Icon> 容器 + slot 自定义 svg，支持五档枚举尺寸（font-size 驱动：8/12/16/20/24），把外部图标纳入统一体系。'),
  entry('02-color-spin.svelte', '语义状态色', '单色具名图标继承外层 color，用 style 应用预设语义色；spin 属性使图标持续旋转，受 reduced-motion 抑制。'),
