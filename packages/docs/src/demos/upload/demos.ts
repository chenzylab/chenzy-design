...
export const demos: DemoEntry[] = [
  entry('01-basic.svelte', '基础用法', 'children 内放 Button（点按钮激活文件选择）与 picture 照片墙加号瓦片两种触发方式（对齐 Semi）。'),
  ...
  entry('05-manual.svelte', '手动上传', 'uploadTrigger="custom" 选文件不自动上传，外部按钮调 ref.upload() 才统一触发（对齐 Semi）。'),
  ...
  entry('08-render-family.svelte', '自定义列表操作区与预览', 'renderFileOperation 自绘预览/下载/删除操作按钮（删除调 file.onRemove）、previewFile 按 uid 返回 IconFile 或缩略图（对齐 Semi）。'),
  ...
  entry('12-preview.svelte', '图片墙放大预览', 'renderPicPreviewIcon 自绘 hover 眼睛图标 + onPreviewClick 触发 ImagePreview 放大预览（对齐 Semi）。'),
  entry('13-default-list.svelte', '默认文件列表', 'defaultFileList 传入已上传项（成功项与 uploadFail 失败项混合态），非受控（对齐 Semi）。'),
  entry('14-disabled.svelte', '禁用', 'disabled 下 list 与 picture 两态：触发器不可点、列表项（含 validateFail 混合态）不可移除。'),
  ...
  entry('30-render-file-item.svelte', '完全自定义列表项', 'renderFileItem 完全自定义文本列表项、showPicInfo + renderPicInfo 图片信息浮层（对齐 Semi）。'),
