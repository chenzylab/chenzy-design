...
export const demos: DemoEntry[] = [
  ...
  entry('10-scale.svelte', '设置缩放值', 'scale=0.5 修正外层 transform:scale 下的拖拽 delta'),
  entry('09-custom-handle.svelte', '自定义把手', 'handleNode 自定义某方向把手内容'),
  entry('16-custom-corner.svelte', '自定义边角样式', 'handleNode / handleStyle / handleClass 自定义边角 handler'),
  entry('04-group-horizontal.svelte', '组合基本使用', 'ResizeGroup/Item/Handler：defaultSize 多单位（400px / 20% / 0.5 / 1）'),
