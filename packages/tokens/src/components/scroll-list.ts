/**
 * Component tokens for ScrollList — 严格对齐 Semi Design
 * （semi-foundation/scrollList/variables.scss 39 变量 + animation.scss 3 变量），
 * 升级为带元数据的 TokenDef 结构以支持 DSM。值为 var() 引用我们的 alias / global token 或字面量。
 *
 * 映射约定（逐条亲验 Semi variables.scss / animation.scss + 我们 alias/global）：
 * - Semi kebab 化：`scrollList` → `scroll-list`，下划线子部件 `_header` → `-header`。
 ...
 * - animation.scss：`--semi-transition_duration-none` → `var(--cd-motion-duration-none)`，
 *   `--semi-transition_function-easeOut` → `var(--cd-motion-ease-standard)`，delay `0ms` 字面量。
 * - transparent / 字面量宽高（16px / 12px / 10px / 1px / 0）保留。
 *
 * 消费方：ScrollList.svelte / ScrollItem.svelte 直接消费 Semi 全名 token（无中间短名）；
 * TimePicker 的 `height-time-picker-scrolllist-item` 复用 `var(--cd-height-scroll-list-item)`。
 */
...
export const scrollListTokens = {
  ...
  // —— Width / Height ——
  'height-scroll-list': { value: '300px', category: 'height', label: '列表高度', usage: '滚动列表 body 高度' },
  'height-scroll-list-item': { value: '36px', category: 'height', label: '选项高度', usage: '滚动列表选项高度（TimePicker 复用）' },
  'width-scroll-list-item-border': { value: 'var(--cd-border-thickness-control)', category: 'width', label: '选项描边宽度', usage: '滚动列表选项列间描边宽度' },
  'width-scroll-list-item-sel-svg': { value: '16px', category: 'width', label: '选中图标宽度', usage: '滚动列表选中图标宽高' },
  'width-scroll-list-item-wheel-border': { value: 'var(--cd-border-thickness-control)', category: 'width', label: '滚轮列间描边宽度', usage: '滚动列表滚轮列间描边宽度' },
  'width-scroll-list-item-wheel-selector-border': { value: 'var(--cd-border-thickness-control)', category: 'width', label: '滚轮选区描边宽度', usage: '滚动列表滚轮选区上下描边宽度' },
  'height-scroll-list-line': { value: '1px', category: 'height', label: 'header 分割线高度', usage: '滚动列表 header 分割线高度' },
  ...
  'spacing-scroll-list-body-paddingy': { value: '0', category: 'spacing', label: '内容垂直内边距', usage: '滚动列表 body 垂直内边距' },
  'spacing-scroll-list-body-paddingx': { value: '16px', category: 'spacing', label: '内容水平内边距', usage: '滚动列表 body 水平内边距' },
  'spacing-scroll-list-item-ul-padding': { value: '0', category: 'spacing', label: '选项列表内边距', usage: '滚动列表选项 ul 内边距' },
  'spacing-scroll-list-item-ul-margin': { value: '0', category: 'spacing', label: '选项列表外边距', usage: '滚动列表选项 ul 外边距' },
  'spacing-scroll-list-item-ul-paddingbottom': { value: 'calc((300px - 36px) * 0.5)', category: 'spacing', label: '选项列表底部内边距', usage: '滚动列表选项 ul 底部内边距（(列表高 - 项高) / 2）' },
  'spacing-scroll-list-item-sel-svg-marginright': { value: '12px', category: 'spacing', label: '选中图标右外边距', usage: '滚动列表选中图标右外边距' },
  'spacing-scroll-list-item-wheel-list-outer-paddingright': { value: 'calc(36px * 0.5)', category: 'spacing', label: '滚轮列表右内边距', usage: '滚动列表滚轮 list-outer 右内边距（项高 / 2）' },
  'spacing-scroll-list-item-wheel-list-shade-pre-margintop': { value: 'calc(-1 * (36px * 0.5 + var(--cd-border-thickness-control)))', category: 'spacing', label: '滚轮上遮罩顶外边距', usage: '滚动列表滚轮上遮罩顶外边距（-(项高 / 2 + 描边宽)）' },
  'spacing-scroll-list-item-wheel-list-shade-post-margintop': { value: 'calc(36px * 0.5 + var(--cd-border-thickness-control))', category: 'spacing', label: '滚轮下遮罩顶外边距', usage: '滚动列表滚轮下遮罩顶外边距（项高 / 2 + 描边宽）' },
  'spacing-scroll-list-footer-padding': { value: '10px', category: 'spacing', label: '底部内边距', usage: '滚动列表 footer 内边距' },
  ...
  // —— Animation（对齐 Semi animation.scss；transition_duration/function/delay） ——
  'transition-duration-scroll-list-selected-item-bg': { value: 'var(--cd-motion-duration-none)', category: 'animation', label: '选中项背景过渡时长', usage: '滚动列表选中项背景色过渡时长' },
  'transition-function-scroll-list-selected-item-bg': { value: 'var(--cd-motion-ease-standard)', category: 'animation', label: '选中项背景过渡曲线', usage: '滚动列表选中项背景色过渡曲线（easeOut）' },
  'transition-delay-scroll-list-selected-item-bg': { value: '0ms', category: 'animation', label: '选中项背景过渡延迟', usage: '滚动列表选中项背景色过渡延迟' },
