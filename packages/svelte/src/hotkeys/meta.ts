...
export const meta = {
  ...
  description:
    '声明一组键盘快捷键组合并绑定 keydown 监听（默认全局 document.body，可局部），命中触发回调，并渲染键位提示（严格对齐 Semi：div.cd-hotKeys > span > span.-content + span.-split，用 span 非 kbd）。修饰键精确匹配 + 普通键用 event.code（物理键位，规避输入法/大小写）。mergeMetaCtrl 为死 prop（严格对齐 Semi：声明但不生效，Meta/Ctrl 仍严格区分）。',
  props: [
