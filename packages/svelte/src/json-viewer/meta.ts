...
export const meta = {
  ...
  props: [
    {
      ...
      type: 'string',
      default: "''",
      desc: '展示内容（JSON 字符串）。非受控：外部传入新字面量会重建实例（对齐 Semi），不建议在 onChange 里回写',
    },
    ...
    {
      name: 'renderTooltip',
      type: '(value: string, el: HTMLElement) => HTMLElement',
      default: 'undefined',
      desc: '自定义 hover tooltip 内容（对齐 Semi）：返回的 HTMLElement 由内核挂载到 tooltip 容器',
    },
  ...
  tokens: [
    ...
    '--cd-color-json-viewer-complete-bg',
    ...
    '--cd-color-json-viewer-search-options-item',
    '--cd-color-json-viewer-search-options-item-active-bg',
    '--cd-color-json-viewer-search-options-item-active-text',
    '--cd-color-json-viewer-search-options-item-hover',
    '--cd-color-json-viewer-toolbar-bg',
    '--cd-color-json-viewer-toolbar-border',
    '--cd-font-json-viewer-mono-fontfamily',
    '--cd-font-json-viewer-mono-fontsize',
    '--cd-radius-json-viewer-complete',
