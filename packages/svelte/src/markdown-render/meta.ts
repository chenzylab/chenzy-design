...
export const meta = {
  ...
  a11y: {
    hasRole: false,
    focusable: false,
    note: '渲染语义化 HTML（标题 / 列表 / 表格 / 链接等）保留原生可访问性；默认剥离 raw HTML 规避注入；图片 alt 由 Markdown 源码 `![alt](url)` 提供。',
  },
  // 默认覆盖元素（对齐 Semi SemiMarkdownComponents 11 键）；pre 不在其中（Semi 无此键，保留原生标签）。
  overridableTags: ['h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'p', 'a', 'img', 'table', 'code'],
  // token 严格镜像 Semi variables.scss（21 条）；排版由 Typography 承担，无自造 h*-size/quote/table/hr 等。
  tokens: [
