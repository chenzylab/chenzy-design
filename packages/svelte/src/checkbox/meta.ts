...
export const meta = {
  ...
  group: {
    ...
    props: [
      ...
      { name: 'aria-label', type: 'string', default: 'undefined', desc: '无可见标题时的可访问名（role=group）' },
      { name: 'ariaLabelledby', type: 'string', default: 'undefined', desc: '关联组可见标题 id（优先于 aria-label）' },
      { name: 'style', type: 'string', default: 'undefined', desc: '根容器内联样式，可设 width 等（对齐 Semi style）' },
