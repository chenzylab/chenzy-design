...
<style>
  /* 逐条对齐 Semi aiChatDialogue.scss &-reasoning。
     注意 Semi 的 scss 里有 &-reasoning（width:100%）这一层，但它的 tsx 从不渲染
     该类名（PREFIX_REASONING 只被 reasoning.tsx 拿去拼 -wrapper 等后缀），
     属 Semi 上游的死 CSS。本库原来照着 scss 多包了一层 div，且与 -wrapper
     同为 width:100%，纯冗余，已删。 */
