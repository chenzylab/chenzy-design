...
export const meta = {
  ...
  props: [
    ...
    { name: 'escapeHtml', type: 'boolean', default: 'true', desc: '是否对用户消息中的 HTML 标签进行转义，防止被 Markdown 解析器当作 HTML 处理导致内容丢失' },
    { name: 'disabledFileItemClick', type: 'boolean', default: 'false', desc: '是否禁用文件点击' },
    { name: 'hintCls', type: 'string', default: "''", desc: '提示区最外层样式类名' },
    { name: 'hintStyle', type: 'string', default: "''", desc: '提示区最外层样式' },
    { name: 'onMessageShare', type: '(message) => void', default: 'undefined', desc: '分享消息回调（传了才渲染分享按钮）' },
    { name: 'onAnnotationClick', type: '(annotations) => void', default: 'undefined', desc: 'annotation 引用标注点击回调（回传整组，已过滤 file_citation）' },
