...
export const meta = {
  ...
  props: [
    ...
    { name: 'currency', type: 'boolean | string', default: 'false', desc: '货币展示：true 按 localeCode 推断币种（未传 localeCode 时读取外层 LocaleProvider），字符串指定 ISO 4217 币种码；仅显示层（对齐 Semi）' },
    { name: 'currencyDisplay', type: "'symbol'|'code'|'name'", default: "'symbol'", desc: '货币展示方式：符号 ￥ / 代码 CNY / 名称 人民币（对齐 Semi）' },
    { name: 'localeCode', type: 'string', default: 'undefined', desc: '货币格式化 BCP-47 locale；回退 locale prop，再回退外层 LocaleProvider 的 code，最后回退 zh-CN（对齐 Semi）' },
