/**
 * Machine-readable component metadata for AI/docs consumption.
 * See specs/00-foundation/ai-friendly.spec.md.
 */
export const meta = {
  name: 'Tooltip',
  category: 'show',
  description:
    '文字提示，鼠标悬停/聚焦/点击触发元素时展示简短说明。支持常用 placement、箭头、延迟、dark/light 主题、status 语义图标与 custom 完全受控触发，浮层命令式定位。',
  props: [
    { name: 'content', type: 'string | Snippet', default: 'undefined', desc: '提示内容，为空不显示' },
    { name: 'open', type: 'boolean', default: 'undefined', desc: '受控显隐' },
    { name: 'defaultOpen', type: 'boolean', default: 'false', desc: '非受控初始显隐' },
    {
      name: 'trigger',
      type: "'hover'|'focus'|'click'|'custom'|Array<'hover'|'focus'|'click'|'custom'>",
      default: "['hover','focus']",
      desc: "触发方式，可组合；'custom' 完全受控（仅 open + onOpenChange，不自动 hover/click/focus）",
    },
    {
      name: 'placement',
      type: "'top'|'topLeft'|'topRight'|'bottom'|'bottomLeft'|'bottomRight'|'left'|'right'",
      default: 'top',
      desc: '浮层方位',
    },
    { name: 'autoAdjustOverflow', type: 'boolean', default: 'true', desc: '视口溢出时翻转到对侧' },
    { name: 'mouseEnterDelay', type: 'number', default: '100', desc: 'hover 进入延迟(ms)' },
    { name: 'mouseLeaveDelay', type: 'number', default: '100', desc: 'hover 离开延迟(ms)' },
    { name: 'showArrow', type: 'boolean', default: 'true', desc: '是否显示箭头' },
    {
      name: 'arrowPointAtCenter',
      type: 'boolean',
      default: 'false',
      desc: 'start/end 对齐时箭头指向触发器中心',
    },
    { name: 'theme', type: "'dark'|'light'", default: 'dark', desc: '主题' },
    {
      name: 'status',
      type: "'default'|'warning'|'error'",
      default: 'default',
      desc: '语义态：非 default 时内容前渲染对应状态图标（图标 aria-label 经 locale）',
    },
    { name: 'maxWidth', type: 'number | string', default: '300', desc: '浮层最大宽度' },
    { name: 'disabled', type: 'boolean', default: 'false', desc: '禁用触发' },
    { name: 'zIndex', type: 'number', default: 'undefined', desc: '浮层 z-index（覆盖 CSS token --cd-tooltip-z）' },
    { name: 'stopPropagation', type: 'boolean', default: 'true', desc: '阻止浮层上的点击事件冒泡' },
    { name: 'preventScroll', type: 'boolean', default: 'false', desc: '显示时阻止页面滚动' },
    {
      name: 'getPopupContainer',
      type: '() => HTMLElement | null | undefined',
      default: 'undefined',
      desc: '自定义浮层挂载容器（缺省回退 ConfigProvider 全局，再回退 body）',
    },
    { name: 'afterClose', type: '() => void', default: 'undefined', desc: '浮层完全关闭（动画结束）后的回调' },
    { name: 'onOpenChange', type: '(open: boolean) => void', default: 'undefined' },
    { name: 'children', type: 'Snippet', default: 'undefined', desc: '触发元素（必填）' },
  ],
  a11y: {
    role: 'tooltip',
    keyboard: ['Escape'],
    notes: [
      '浮层 role=tooltip，使用 useId 生成唯一 id',
      '触发元素 open 时加 aria-describedby 指向浮层',
      'focus 触发支持键盘聚焦展示，click 触发用 useDismiss 处理 Escape 与外部点击',
      'custom 触发不绑定 hover/click/focus，显隐完全由受控 open + onOpenChange 控制',
      'status 非 default 时状态图标用 role=img + aria-label（经 locale）',
    ],
  },
  tokens: [
    '--cd-tooltip-bg-dark',
    '--cd-tooltip-color-dark',
    '--cd-tooltip-bg-light',
    '--cd-tooltip-color-light',
    '--cd-tooltip-radius',
    '--cd-tooltip-padding',
    '--cd-tooltip-font-size',
    '--cd-tooltip-line-height',
    '--cd-tooltip-shadow',
    '--cd-tooltip-z',
    '--cd-tooltip-arrow-size',
    '--cd-tooltip-status-gap',
    '--cd-tooltip-status-warning',
    '--cd-tooltip-status-error',
    '--cd-tooltip-*',
    '--cd-focus-ring',
    '--cd-motion-*',
  ],
} as const;
