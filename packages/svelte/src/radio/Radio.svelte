<!--
  Radio — 严格对齐 Semi radio.tsx，内层圆圈拆到 RadioInner.svelte（对齐 radioInner.tsx）。

  DOM 结构镜像 Semi：
    <label class="cd-radio ...">
      <RadioInner ... />                            ← span.cd-radio-inner > input + inner-display
...
<script lang="ts">
  ...
  import RadioInner from './RadioInner.svelte';
  import {
  ...
  let innerEntity = $state<RadioInner | null>(null);
  $effect(() => {
    if (autoFocus) innerEntity?.focus({ preventScroll });
  });

  /** Imperatively focus the radio's input (mirrors Semi's `focus()`). */
  export function focus(): void {
    innerEntity?.focus({ preventScroll });
  }
...
>
  <RadioInner
    ...
    onInputBlur={() => (focusVisible = false)}
  />
  {#if hasContent}
...
<style>
  ...
  /* inner/input/inner-display 现在渲染自子组件 RadioInner.svelte，基础规则随 DOM 迁移到那边
     （Svelte scoped CSS 按文件生效，父组件里写这些选择器不会命中子组件渲染的元素）。此处仅保留
     跨组件边界的组合选择器（:hover 等状态类在父 label 上，需 :global() 打透子组件的 inner-display）。 */
  .cd-radio-disabled :global(.cd-radio-native-control) {
    cursor: not-allowed;
  }

  /* ---- 未选中态 hover / active（对齐 Semi &:hover / &:active .inner-display） ---- */
  .cd-radio:hover :global(.cd-radio-inner-display) {
  ...
  .cd-radio:active :global(.cd-radio-inner-display) {
    background: var(--cd-color-radio-default-bg-active);
  }
  /* 选中态 hover / active（品牌色描边随之变化） */
  .cd-radio:hover :global(.cd-radio-inner-checked .cd-radio-inner-display) {
    background: var(--cd-color-radio-primary-bg-hover);
    border-color: var(--cd-color-radio-checked-border-hover);
  }
  .cd-radio:active :global(.cd-radio-inner-checked .cd-radio-inner-display) {
  ...
  .cd-radio-disabled :global(.cd-radio-inner) {
    cursor: not-allowed;
  }
  .cd-radio-disabled :global(.cd-radio-inner-display) {
    opacity: 0.75;
    background: var(--cd-color-radio-disabled-bg-default);
    border-color: var(--cd-color-radio-default-border-disabled);
  }
  .cd-radio-disabled :global(.cd-radio-inner-display):hover {
    background: var(--cd-color-radio-disabled-bg-hover);
  }
  .cd-radio-disabled :global(.cd-radio-inner-checked .cd-radio-inner-display) {
  ...
  .cd-radio-disabled:hover :global(.cd-radio-inner-display),
  .cd-radio-disabled:active :global(.cd-radio-inner-display) {
    background: var(--cd-color-radio-disabled-bg-default);
    border: solid var(--cd-width-radio-disabled-border) var(--cd-color-radio-disabled-border-default);
  }
  .cd-radio-disabled:hover :global(.cd-radio-inner-checked .cd-radio-inner-display),
  .cd-radio-disabled:active :global(.cd-radio-inner-checked .cd-radio-inner-display) {
    color: var(--cd-color-radio-checked-icon-disabled);
    background: var(--cd-color-radio-checked-bg-disabled);
    border-color: var(--cd-color-radio-checked-border-disabled);
  }
  ...
  .cd-radio-cardRadioGroup :global(.cd-radio-inner) {
    flex-shrink: 0;
  }
  /* card 型未选中圆圈用卡片默认底（对齐 Semi &.cardRadioGroup .inner .inner-display）；
     选中圆圈保持 .inner-checked 的品牌蓝底，故此处排除 checked，避免白底盖掉蓝底。 */
  .cd-radio-cardRadioGroup :global(.cd-radio-inner:not(.cd-radio-inner-checked) .cd-radio-inner-display) {
  ...
  .cd-radio-cardRadioGroup:hover :global(.cd-radio-inner-display) {
    background: var(--cd-color-radio-card-bg-hover);
  }
  .cd-radio-cardRadioGroup:active :global(.cd-radio-inner-display) {
  ...
  .cd-radio-cardRadioGroup_checked:hover :global(.cd-radio-inner-checked .cd-radio-inner-display) {
  ...
  .cd-radio-cardRadioGroup_checked:active :global(.cd-radio-inner-checked .cd-radio-inner-display) {
  ...
  .cd-radio-cardRadioGroup_checked_disabled :global(.cd-radio-inner-checked .cd-radio-inner-display) {
    border-color: var(--cd-color-radio-cardradiogroup-checked-disabled-border-default);
  }
  .cd-radio-cardRadioGroup_checked_disabled:hover :global(.cd-radio-inner-checked .cd-radio-inner-display) {
