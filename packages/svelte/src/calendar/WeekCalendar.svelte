<!--
  WeekCalendar — 周视图，对齐 Semi `semi-ui/calendar/weekCalendar.tsx`。
  DOM：cd-calendar-week > week-sticky-top(header + week-header + all-day) + week-scroll-wrapper > week-scroll > (TimeCol + 7×DayCol)。
  从 Calendar.svelte 的 mode==='week' 分支原样迁出，逻辑不变；本视图独立管理自己的响应式状态（不与其他视图共享 script 作用域）。
  对齐 Semi：weekCalendar.tsx 与 rangeCalendar.tsx 是完全独立重复实现，不共享代码，故本文件与 RangeCalendar.svelte 各自持有一份相同的 CSS。
-->
<script lang="ts">
  ...
  import type { WeekCalendarProps } from './types.js';
  ...
  const spanEvents = $derived(parseWeekSpanEvents(allDayBucket, startOfWeek(anchor, weekStartsOn), weekStartsOn));
...
<!-- ===== week 视图（对齐 Semi weekCalendar：根无 role）===== -->
<div
