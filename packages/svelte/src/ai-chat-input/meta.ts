 ...
 * 阶段 5（Adapter 桥）：chatInputToMessage / chatInputToChatCompletion 把 onMessageSend 载荷转成
...
export const meta = {
  ...
  props: [
    ...
    { name: 'round', type: 'boolean', default: 'true', desc: '圆角样式（对齐 Semi round）' },
  ...
  slots: [
    ...
    { name: 'renderTopSlot', payload: '{ references, attachments, handleReferenceDelete, handleUploadFileDelete }', desc: '自定义 top slot 渲染（阶段 2）' },
  ...
  examples: [
    ...
    { title: '接入对话', desc: 'chatInputToMessage 把发送载荷转 Message 喂 AIChatDialogue（Adapter 桥）' },
