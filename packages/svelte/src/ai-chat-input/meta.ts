 ...
 * 阶段 4+（configure/adapter）见 spec §6。
 */
export const meta = {
  name: 'AIChatInput',
  category: 'show',
  description:
    'AI 聊天输入框：基于 tiptap 富文本编辑器（内核 + svelte-tiptap 动态 import 懒加载，绝不进主 bundle）。阶段 1 富文本输入 + 发送（sendHotKey、canSend、generating 停止键）+ Upload 附件。阶段 2 references 引用条（可点击/删除/renderReference 覆盖）+ suggestions 建议浮层（聚焦弹出，↑↓/Enter/Esc/点击外部，renderSuggestionItem 覆盖）+ renderTopSlot。阶段 3 skills 技能面板（空编辑区按 skillHotKey 弹出，↑↓/Enter/Esc，选中插入 skill-slot 自定义节点并 onSkillChange）+ skill-slot NodeView（inline atom chip，含删除，用 svelte-tiptap SvelteNodeViewRenderer 渲染 SkillSlotNode.svelte）+ renderTemplate 模版面板（当前技能 hasTemplate 时展示模版按钮，点击弹出，setContent 填入）。onMessageSend 载荷对齐 Semi MessageContent（inputContents/attachments/references）。headless（canSend/sendHotKey/MessageContent/doc→contents/suggestion 键盘/reference 归一/skill 归一/getSkillSlotHTML/skillHotKey 判定）在 @chenzy-design/core，可单测。ref 方法 setContent/focusEditor/getText/getHTML/getEditor/clearContent/changeTemplateVisible。configure/adapter 为 P4+。',
  exports: ['AIChatInput'],
  props: [
  ...
  tokens: ['--cd-ai-chat-input-*（容器/编辑区/占位符/上传图标/发送-停止/引用条/建议-技能面板/skill-slot chip/模版按钮，深浅双主题）'],
  examples: [
