<script lang="ts">
  ...
  let { fill, ...rest }: Props = $props();

  const id = getUuidShort({ prefix: 'semi-ai-bell-level-3' });
  const [stop1, stop2, stop3, stop4] = $derived.by(() => getFillColor(fill, 4));
</script>

<!-- IconAIBellLevel3 — AI 多彩图标，对齐 Semi IconAIBellLevel3（convertIcon 包 Icon 基座，type='ai_bell_level_3'）。fill 数组自定义多色，size/spin/rotate 由 Icon 基座承载。 -->
<Icon type="ai_bell_level_3" {...rest}>
<svg
            viewBox="0 0 24 24"
            fill="none"
        ...
        >
            ...
            <defs>
                ...
                >
                    ...
                    <stop offset="0.3" stop-color={stop2} />
                    <stop offset="0.6" stop-color={stop3} />
                    <stop offset="1" stop-color={stop4} />
                </linearGradient>
            </defs>
        </svg>
</Icon>
